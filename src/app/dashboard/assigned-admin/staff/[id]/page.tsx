// src/app/dashboard/assigned-admin/staff/[id]/page.tsx
...
export default async function StaffDetailPortalPage({ params }: PageProps) {
  const resolvedParams = await params;
  const targetStaffId = resolvedParams.id;

  // 1. Theme Cookie Check for Server-side Rendering match
  ...
  // 3. Strict Boundary Check: Verify an APPROVED connection exists between this Admin and target Staff
  const connection = await prisma.adminStaffConnection.findFirst({
    where: {
  ...
  if (!connection) {
    return (
      <div className={`p-8 text-center text-red-400 space-y-4 max-w-md mx-auto mt-20 border rounded-2xl ${
        isDarkMode ? 'bg-[#1c2541] border-red-500/30' : 'bg-white border-red-200 shadow-xl'
      }`}>
        <h1 className="text-lg font-bold">Unauthorized Access</h1>
        <p className={`text-xs ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
          You do not have an active, approved administrative connection with this staff member.
        </p>
        <Link
          href="/dashboard/assigned-admin"
          className="inline-block bg-blue-600 text-white text-xs px-4 py-2 rounded-xl transition hover:bg-blue-500"
        >
          ← Return to Portal
        </Link>
      </div>
  ...
  let receptionistConnections: any[] = [];
  let receptionistActivity: any[] = [];

  if (isDoctor) {
    try {
      doctorVisits = await prisma.visit.findMany({
      ...
      doctorVisits.forEach((visit) => {
        const collected = visit.feePaid ?? visit.consultationFee ?? 0;
        const dues = visit.dues ?? 0;
        ...
        if (visitDate.toDateString() === todayString) {
          dailyEarnings += collected;
        }

        if (visitDate.getFullYear() === currentYear) {
          annualEarnings += collected;
        }
      });
    } catch (e) {
      console.error('Failed to fetch doctor visits:', e);
      doctorVisits = [];
    }
  } else if (isReceptionist) {
    try {
      receptionistConnections = await prisma.staffConnection.findMany({
        where: {
      ...
      const connectedDoctorIds = receptionistConnections.map((conn) => conn.doctorId);

      if (connectedDoctorIds.length > 0) {
        receptionistActivity = await prisma.visit.findMany({
          where: {
            doctorId: { in: connectedDoctorIds },
          },
      ...
      console.error('Failed to fetch receptionist activity:', e);
    ...
    <div className={`min-h-screen flex flex-col transition-colors duration-300 p-6 space-y-6 max-w-7xl mx-auto ${
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Viewing records and professional data under your administrative oversight.</p>
        </div>
